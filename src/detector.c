...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    FILE* train_log_file;
    train_log_file = fopen("train_log.txt", "w+");
    ...
    fprintf(train_log_file, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
    data train, buffer;
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        fprintf(train_log_file, "\n %d: %f, %f avg loss, %f rate, %lf seconds, %d images\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), (what_time_is_it_now() - time), iteration* imgs);
        fflush(stdout);
...
{
    FILE* valid_log = fopen("valid_log.txt", "a+");
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", NULL);
    ...
    for (i = 0; i < classes; ++i) {
        ...
        fprintf(valid_log, "class_id = %d, name = %s, \t ap = %2.2f %% (TP = %d, FP = %d) \n", i, names[i], avg_precision * 100, tp_for_thresh_per_class[i], fp_for_thresh_per_class[i]);
    ...
    fprintf(valid_log, "mean average precision (mAP@%0.2f) = %f, or %2.2f %% \n", iou_thresh, mean_average_precision, mean_average_precision * 100);
    ...
    float hier_thresh, int dont_show, char* outfile, int save_labels, char* in_folder, char* out_folder, char * cut_folder, char * raw_folder, int out_term)
{
    ...
    network net = parse_network_cfg_custom(cfgfile, 1,1); // yolov3-tiny_5mp.cfg ve türevlerini veriyoruz
    if (weightfile) {
        load_weights(&net, weightfile);
    }
    //set_batch_network(&net, 1);
    ...
    if (in_folder)
    {
        ...
        // -------- Vincent: start loop per file -----------
                ...
                if (multi_thresh != NULL)
                {
                    ...
                    for (int i = 0; i < multi_count; i++)
                    {
                        ...
                        if (out_folder) { // data/SATA/out_images/ 
                            ...
                            ftoa(multi_thresh[i], temp, 3);
                            int copylength = strlen(out_folder) - 1; 
                        ...
                        if (save_labels)
                        {
                            ...
                            for (i = 0; i < nboxes; ++i) {
                                ...
                                for (j = 0; j < l.classes; ++j) {
                                    if (dets[i].prob[j] > multi_thresh[i] && dets[i].prob[j] > prob) {
                                        prob = dets[i].prob[j];
                                        class_id = j;
                                    }
                                }
                ...
                else
                {
                    ...
                    draw_detections_v3(filename, im, dets, nboxes, thresh, names, alphabet, l.classes, outfile, out_term);
                    ...
                    if (cut_folder)
                    {
                        ...
                        {
                            char * folder = concat(cut_folder, names[i]);
                            mkdir(folder);
                        }
                        if (raw_folder)
                        {
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers, int out_term)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
...
void draw_object(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, int dont_show, int it_num,
    int letter_box, int benchmark_layers, int outfile, int out_term)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
...
void run_detector(int argc, char **argv)
{
    ...
    float* multi_thresh = find_multi_float_arg(argc, argv, "-multi_thresh");
    ...
    char* outfile = find_char_arg(argc, argv, "-out", 0); // results/result.txt
    char* filename = find_char_arg(argc, argv, "-filename", 0); // işlencek resim dosyası adı (tek resim için çalıştırırken)
    char* in_folder = find_char_arg(argc, argv, "-in_folder", 0); // data/SATA/in_images/
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers, out_term);
        ...
        int it_num = 100;
        draw_object(datacfg, cfg, weights, filename, thresh, dont_show, it_num, letter_box, benchmark_layers, outfile, out_term);
    }
