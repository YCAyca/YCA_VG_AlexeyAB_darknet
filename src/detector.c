    ...
    float hier_thresh, int dont_show, char * outfile, int save_labels, char *in_folder, char *out_folder, int out_term)
{
    ...
    network net = parse_network_cfg_custom(cfgfile, 1); // yolov3-tiny_5mp.cfg ve türevlerini veriyoruz
    if (weightfile) {
        load_weights(&net, weightfile);
    }
    //set_batch_network(&net, 1);
                    ...
                    for(int i = 0; i < multi_count; i++)
                    {
                     ...
                     //    printf("%f", multi_thresh[i]);
                         ...
                         if (out_folder) {
                          ...
                          //   printf("copylength : %d ", copylength);

                             memset(temp2, '\0', sizeof(temp2));
                        
                             const * out = strncpy(temp2, out_folder, copylength);
                          //   printf("ad : %s \n", out);
                             char* temp3 = concat(temp2, temp);
                             char * outfoldername = concat(temp3, "/");
                             mkdir(outfoldername);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, char * outfile, float thresh,
                   float hier_thresh, int dont_show, int save_labels, int out_term)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
...
void run_detector(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "batch"))
        batch_detector(datacfg, cfg, weights, filename, multi_thresh, thresh, hier_thresh, dont_show, outfile, save_labels, in_folder, out_folder, out_term);
    else
    //------ vincent end --------
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, outfile, thresh, hier_thresh, dont_show, save_labels, out_term);
